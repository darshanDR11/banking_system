...
select f.film_id, title, count(rental_id) as total_rented 
from rental as r 
...
select f.film_id, title, sum(amount) as total_revenue
from payment as p
...
-- 100. Show film titles along with their language names.
select film_id, title, f.language_id, name 
from film as f 
join language as l on f.language_id = l.language_id
order by film_id;
...
select f.film_id, title, c.customer_id, first_name, last_name
from rental as r 
join inventory as i on r.inventory_id = i.inventory_id
join film as f on i.film_id = f.film_id
...
-- 107. Display each customer with the total number of rentals they made (include customers with zero rentals).
select c.customer_id, count(rental_id) as total_rented 
from customer as c 
...
-- 112. Show film titles along with the total payment collected for each film.
select f.film_id, title, sum(amount) as total_payment
from payment as p
join rental as r on p.rental_id = r.rental_id
join inventory as i on r.inventory_id = i.inventory_id
join film as f on i.film_id = f.film_id
...
-- 115.Show staff with total rentals handled
select s.staff_id, first_name, last_name, count(rental_id) as total_rental 
from rental as r 
...
-- 119 Display films with no inventory (LEFT JOIN logic)
...
-- 127 Show customers and number of payments made
select c.customer_id, count(payment_id) as total_payment 
from customer as c 
...
select r.rental_id, rental_date, inventory_id, return_date, customer_id, r.staff_id, first_name, last_name 
from rental as r 
join staff as s on r.staff_id = s.staff_id order by r.rental_id;
...
-- 133 Display films with maximum rental duration per category
select fc.category_id, max(rental_duration) as maximum_rental_duration
from film as f 
join film_category as fc on f.film_id = fc.film_id 
...
select f.film_id, title, round(avg(amount),2) as average_payment
from payment as p
join rental as r on p.rental_id = r.rental_id
join inventory as i on r.inventory_id = i.inventory_id
join film as f on i.film_id = f.film_id
...
-- 135 Show average payment per store.
select s.store_id, round(avg(amount),2) as average_payment
from payment as p
join staff as s on p.staff_id = s.staff_id
group by s.store_id;
...
select customer_id from payment group by customer_id having avg(amount) > (select avg(amount) from payment);
...
-- 145. Find customers who rented films above average length.
...
select * from customer where not customer_id in (select distinct customer_id from payment);

-- 151. Find films with above average replacement cost
...
select film_id, title from film where film_id = (
								select film_id from payment as p 
                                join rental as r on p.rental_id = r.rental_id 
                                join inventory as i on r.inventory_id = i.inventory_id 
								group by film_id order by sum(amount) desc limit 1);
                                
-- 154. Find customers who rented films with the highest replacement cost.
select * from customer where customer_id in (
									select distinct customer_id
									from rental as r 
									join inventory as i on r.inventory_id = i.inventory_id 
									join film as f on i.film_id = f.film_id where replacement_cost = (select max(replacement_cost) from film));

-- 155. Find films with lowest rental rate.
select film_id, title, release_year, length, rating, rental_rate 
from film 
where rental_rate = (select min(rental_rate) from film);

-- 156. Find categories with highest rental count.
select * from category where category_id = (
										select category_id from film_category as f 
                                        join inventory as i on f.film_id = i.film_id 
                                        join rental as r on i.inventory_id = r.inventory_id 
										group by f.category_id order by count(rental_id) desc limit 1);
...
-- 158. Find categories with lowest average film length.
select * from category where category_id = (
								select category_id from film_category as fc 
...
group by s.store_id, r.staff_id
)
